...
inst_tuv4(){
    ...
    wget https://raw.githubusercontent.com/yjyygydx/test-public/main/tuic/files/tuic-latest-linux-$(archAffix) -O /usr/local/bin/tuic
...
EOF
    ...
    url="tuic://$domain:$port?password=$token&alpn=h3&mode=bbr#tuic-mzlwl"
    echo ${url} > /root/tuic/url.txt

    cat << EOF >/etc/systemd/system/tuic.service
[Unit]
Description=tuic Service
Documentation=https://github.com/yjyygydx/test-public/tree/main/tuic
...
inst_tuv5(){
    ...
    wget https://raw.githubusercontent.com/yjyygydx/test-public/main/tuic/files/tuic-server-latest-linux-$(archAffix) -O /usr/local/bin/tuic
    if [[ -f "/usr/local/bin/tuic" ]]; then
        chmod +x /usr/local/bin/tuic
    else
        red "Tuic V5 内核安装失败！"
...
EOF
    ...
    url="tuic://$uuid:$passwd@$domain:$port?congestion_control=bbr&udp_relay_mode=quic&alpn=h3#tuicv5-mzlwl"
    echo ${url} > /root/tuic/url.txt
